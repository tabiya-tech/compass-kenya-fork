...
describe("CreateUserModal role select", () => {
  ...
  test("shows the password reset link after the user is created", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId(DATA_TEST_ID.CREATE_USER_MODAL_RESET_LINK)).toBeInTheDocument());
    expect(screen.getByTestId(DATA_TEST_ID.CREATE_USER_MODAL_RESET_LINK)).toHaveValue(givenLink);
